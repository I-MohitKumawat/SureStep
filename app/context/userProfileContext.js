...
export function UserProfileProvider({ children }) {
  ...
  const loadProfileForRole = React.useCallback(async (roleSlug) => {
    if (!roleSlug) return;
    // Key includes BOTH role AND phone so different users on the same device are isolated
    const phone = await AsyncStorage.getItem('current_phone');
    const storageKey = phone
      ? `${STORAGE_KEY_PREFIX}${roleSlug}_${phone}`
      : `${STORAGE_KEY_PREFIX}${roleSlug}`;
  ...
  const saveProfile = React.useCallback(
    async (draft) => {
      ...
      const phone = await AsyncStorage.getItem('current_phone');
      const key = phone
        ? `${STORAGE_KEY_PREFIX}${draft.role}_${phone}`
        : `${STORAGE_KEY_PREFIX}${draft.role}`;
    ...
    setHasProfileForRouting(true);
      try {
        ...
        // Automatically sync caregiver profiles to the backend search directory
        if (draft.role === 'caregiver') {
            const phone = await AsyncStorage.getItem('current_phone') || `temp-${Date.now()}`;
            await supabase.from('caregivers').upsert({
              id: phone,
              name: draft.fullName || 'Anonymous',
              specialty: draft.caregiverInfo?.relationship || 'Family',
              bio: draft.caregiverInfo?.experience || ''
            });
        }
